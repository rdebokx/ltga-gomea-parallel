...
public class PPInstancesPopSizeDeterminator extends PopSizeDeterminator{
    ...
    public static void main(String[] args) {
        ...
        if(configs != null){
            ...
            SequentialPopSizeDeterminator.printStatistics("PPInstancesPopSizeDeterminator", configs[0], popSizes, evaluations);
        } else {
    ...
    private static boolean tryPopSize(){
        ...
        while(runsCount < runs && fails < 2){
            ...
            ParallelJobRunner runner = new ParallelJobRunner(configs[runsCount], false);
            runner.run();
            Solution best = runner.getBestFound();
            InstancesConfiguration instancesConfig = (InstancesConfiguration) configs[runsCount].PROBLEM_CONFIG;
            //System.out.println("Config: " + instancesConfig);
            //System.out.println("Best solution found: " + best);
            
            if(instancesConfig.OPTIMAL_SOLUTION != null && !Arrays.equals(instancesConfig.OPTIMAL_SOLUTION, best.getSolution())
            		|| instancesConfig.OPTIMAL_SOLUTION == null && instancesConfig.OPTIMAL_VALUE - best.getObjectiveValue() > 0.0001){
    ...
    /**
     * This function gathers evaluation statistics for the current configuration, determined over 100 runs.
    ...
    private static int[] getEvaluationsStatistics(){
        ...
        for(int i = 0; i < runs; i++){
            ParallelJobRunner runner = new ParallelJobRunner(configs[i], false);
            runner.run();
