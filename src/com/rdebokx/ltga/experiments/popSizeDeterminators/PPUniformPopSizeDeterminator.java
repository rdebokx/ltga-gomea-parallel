...
public class PPUniformPopSizeDeterminator extends PopSizeDeterminator{
    ...
    public static void main(String[] args) {
        ...
        if(config != null){
            ...
            */
            ...
            SequentialPopSizeDeterminator.printStatistics("PPPopSizeDeterminator", config, popSizes, evaluations);
        } else {
    ...
    private static JobConfiguration loadJobConfiguration(String[] args) {
        ...
        if(args.length == 4){
            ...
            System.out.println("Arguments could not be parsed. Please provide problem | numberOfParameters | inputFile | Threads");
        }
    ...
    private static boolean tryPopSize(JobConfiguration jobConfig){
        ...
        boolean[] optimalSolution = ((InstancesConfiguration) jobConfig.PROBLEM_CONFIG).OPTIMAL_SOLUTION;
        double optimalValue = ((InstancesConfiguration) jobConfig.PROBLEM_CONFIG).OPTIMAL_VALUE;
        ...
        while(runs < RUNS && fails < 2){
            ...
            if(optimalSolution != null && !Arrays.equals(optimalSolution, best.getSolution())
            		|| optimalSolution == null && optimalValue - best.getObjectiveValue() > 0.0001){
    ...
    private static int doBinSearch(int searchId, int start, int end, JobConfiguration jobConfig){
        ...
        if(end - start <= 1){
	        ...
	        System.out.println("Doing binSearch at " + middle);
	        int newStart;
	        int newEnd;
    ...
    /**
     * This function returns an array with evaluations made for 100 runs with the given configuration.
     * @param config The configuration for which evaluation statistics have to be gathered.
     * @return The array with the 100 evaluation values for the given configuration.
     */
    private static int[] getEvaluationsStatistics(JobConfiguration config){
        int[] evaluations = new int[RUNS];
        for(int i = 0; i < RUNS; i++){
            ParallelJobRunner runner = new ParallelJobRunner(config, false);
            runner.run();
