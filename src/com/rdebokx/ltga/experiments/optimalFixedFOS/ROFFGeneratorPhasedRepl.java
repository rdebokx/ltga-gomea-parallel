...
public class ROFFGeneratorPhasedRepl {
	...
	public static Collection<ParameterSet> getCombinations(OptimalFixedFOSConfiguration config, int numberOfParameters){
		...
		while(oldSize < result.size()){
		    ...
		    for(int i = 0; i < oldSize; i++){
		    	System.out.println("i=" + i + ", result.size=" + result.size() + " newSets.size=" + newSets.size());
		    	...
		    	for(int j = i; j < oldSize; j++){
		    		...
		    		if(ROFFGenerator.getScore(config, newFOS) < THRESHOLD * ROFFGenerator.getScore(config, result) && !newSets.contains(combinedSet) && !result.contains(combinedSet)){
