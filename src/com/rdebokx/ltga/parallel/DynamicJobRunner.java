...
public class DynamicJobRunner extends JobRunner {
     ...
     */
    public DynamicJobRunner(JobConfiguration config, JobConfiguration runnersConfig, boolean printResults) {
        super(config, new JobState(), printResults, new Randomizer());
        this.currentConfig = runnersConfig;
    }
     ...
     * @param config The configuration for the job to be run.
     */
    public DynamicJobRunner(JobConfiguration config, JobConfiguration runnersConfig, boolean printResults, long seed) {
     ...
     * @param config The configuration for the job to be run.
     */
    public DynamicJobRunner(JobConfiguration config, JobConfiguration runnersConfig, boolean printResults, LearningModel learningModel, long seed) {
    ...
    public void run() {
        ...
        while(!stopRunner){
            ...
            ParallelJobRunner runner = new ParallelJobRunner(currentConfig, false, this.fixedFOS, this.randomizer.generator.nextLong());
            runner.run();
            if(printResults){
	...
	public JobRunner getNewRunnerWithConfig(JobConfiguration config) {
		return new DynamicJobRunner(config, currentConfig, false);
	}
